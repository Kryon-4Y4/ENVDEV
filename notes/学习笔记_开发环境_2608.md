...
- [六、前后端分离三端架构](#sec-6) —— 快速开始、Core/Server/UI、CORS、FastAPI 概念、历史污染防御、流式输出、React 前端工程化
...
└── project-agentic/                   # 项目框架（内层，可独立打包）
    ...
    └── envdev/                        # 主代码包（包名必须小写合法）
        ...
        ├── api_gateway/               # API 域：FastAPI 服务（:8000）
...
| `core/core.py` | 业务核心：读 AGENT.md + skill 组装 system prompt，封装 LLM 调用，三端共用 |
...
| 终端 | 命令 | 效果 |
|------|------|------|
| 终端 1（API） | `python -m envdev.api_gateway.server` | API 服务 :8000（文档 :8000/docs） |
| 终端 2（前端） | `cd project-agentic/envdev/user_portal/webui && npm run dev` | React 聊天页 :5173（代理 /api → :8000） |
| 终端 3（CLI） | `python -m envdev` | 终端对话，`exit` 退出 |


### 6.1 一句话本质

**Core 是库（被 import，不是独立进程），Server 和 UI 才是两个独立进程**，通过 HTTP + CORS 通信：

| 层 | 代码 | 形态 | 端口 |
|----|------|------|------|
| Core | `core/core.py` | 业务库（被 import） | 无 |
| Server | `api_gateway/server.py` | FastAPI 纯 API，不托管页面 | :8000 |
| UI | `user_portal/webui/`（React 工程） | 开发：Vite 服务器 | :5173 |
...
python -m envdev.api_gateway.server       # API 端 :8000（API 文档：:8000/docs）
cd envdev/user_portal/webui && npm run dev   # 前端 React 版 :5173（代理 /api → :8000）
```
...
| 前端 | fetch 用绝对地址：`` `http://${location.hostname}:8000/api/chat` ``（兼容 127.0.0.1 / localhost） |
...
**开发模式无 CORS 烦恼**：前端写相对路径 `/api/chat/stream`，Vite 代理把请求转发到 :8000（同源转发，浏览器不拦）；生产模式（build + preview/静态托管）才回到 CORS 方案。
