...
| 平台 | 部署内容 | 域名 | 说明 |
|------|---------|------|------|
| GitHub | 整个 ENVDEV 仓库 | — | 代码托管，CI/CD 源头 |
| Vercel | `project-agentic/envdev/user_portal/webui/` | `xxx.vercel.app`（默认） | 纯静态前端，`vercel.json` 控制构建 |
| Railway | 整个 Python 项目 | `xxx.railway.app`（默认） | FastAPI 后端（含 Core），`.env` 在平台配 |
...
1. GitHub 创建 repo → 本地 `git remote add` → push
2. Railway 新建项目 → "Deploy from GitHub repo" → 选 ENVDEV
3. 配置 Railway：
...
5. 把 Railway 域名填回 `vercel.json` 的 `rewrites`
...
allow_origins=[
    ...
    "https://xxx.vercel.app",          # Vercel 生产域名（替换为实际值）
],
```
...
```bash
# ① GitHub 创建 repo 后，本地添加 remote
cd /path/to/ENVDEV
git remote add origin https://github.com/Kryon-4Y4/ENVDEV.git
...
- **改代码后自动部署**：GitHub push → Vercel 自动重新构建前端；Railway 自动重新部署后端
