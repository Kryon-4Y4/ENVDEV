...
- [六、前后端分离三端架构](#sec-6) —— 快速开始、Core/Server/UI、CORS、FastAPI 概念、历史污染防御、流式输出
...
└── project-agentic/                   # 项目框架（内层，可独立打包）
    ...
    └── envdev/                        # 主代码包（包名必须小写合法）
        ...
        │   └── webui/front.py + static/  #  前端静态托管（:3000）
...
| `user_portal/webui/front.py` | 独立静态服务，托管聊天页面（真正前后端分离） |
...
| 终端 2（前端） | `python -m envdev.user_portal.webui.front` | 聊天页面 :3000 |
...
| UI | `user_portal/webui/front.py` + `static/` | 独立静态服务 | :3000 |
...
python -m envdev.user_portal.webui.front  # 前端 :3000（需先启动 API 端）
```
