...
- [七、Agent 人设与技能](#sec-7) —— AGENT.md vs Skill.md、二维码在 Markdown 预览中的等宽渲染
...
| `user_portal/gui/main.py` | 桌面窗口聊天（Tkinter 标准库，直调 core，见 6.9） |
| `user_portal/mobile_app/` | 手机 APP（Expo + React Native）：`npx expo start` 出二维码，手机装 Expo Go 扫码运行，见 6.10 |
...
**备选方案**：如果 ChatBot 的网页 UI 不支持 HTML 渲染，直接给可点击链接更可靠：

```markdown
TG链接：https://t.me/KuangyueHuang
```
...
| Vercel | `project-agentic/envdev/user_portal/webui/` | [envdev-tau.vercel.app](https://envdev-tau.vercel.app) | ✅ 已上线，push 自动部署 |
| Railway | 整个 Python 项目 | 待定 | ⏳ 待部署 |
...
// vercel.json（放在项目根目录）
{
  ...
  "rewrites": [
    { "source": "/api/:path*", "destination": "https://envdev-production.up.railway.app/api/:path*" }
  ]
}
```
...
> **当前状态**：`rewrites` 中的 `YOUR_RAILWAY_URL` 是占位符，等 Railway 后端部署后替换为实际域名。在此之前前端页面可正常访问，但 API 请求不会生效。

### 8.4 后端部署（Railway）—— ⏳ 待执行

**步骤**：

1. ~~GitHub 创建 repo → 本地 push~~ ✅ 已完成
2. Railway 新建项目 → "Deploy from GitHub repo" → 选 `Kryon-4Y4/ENVDEV`
3. 配置 Railway：
   - **Root Directory**：`project-agentic`（`pyproject.toml` 所在位置）
   - **Start Command**：`python -m envdev.api_gateway.server`
   - **Variables**：在 Railway 面板设置 `DEEPSEEK_API_KEY`、`LLM_MODEL` 等（替代本地 `.env`）
4. Railway 自动分配域名（如 `envdev-production.up.railway.app`）
5. 把 Railway 域名填回 `vercel.json` 的 `rewrites`（替换 `YOUR_RAILWAY_URL`）
...
allow_origins=[
    ...
    "https://envdev-tau.vercel.app",   # Vercel 生产域名（已确认）
],
```
...
# ⑤ 回填域名 ⏳ 待执行
#    把 Railway 分配的域名填入 vercel.json 的 rewrites
#    把 Vercel 域名填入 server.py 的 CORS allow_origins
```
...
| Git 连接失败 | `vercel git connect` 报权限错误 | CLI 连不上，需要在 Vercel 网页端 Settings → Git 手动连接 GitHub 仓库 |
...
### 8.9 待完成
待完成：Railway 后端部署
前端页面已经上线，但 API 还不能工作——因为后端（FastAPI + Core）还没部署。需要你手动操作：
打开 https://railway.app，用 GitHub 登录
New Project → Deploy from GitHub repo → 选 Kryon-4Y4/ENVDEV
配置：
Root Directory：project-agentic
Start Command：python -m envdev.api_gateway.server
在 Variables 面板添加环境变量：
DEEPSEEK_API_KEY = 你的 API 密钥
LLM_MODEL = 你的模型名
Railway 会自动分配一个域名（如 envdev-production.up.railway.app）
拿到 Railway 域名后告诉我，我来：
更新 vercel.json 的 rewrites（让前端 API 请求转发到后端）
更新 server.py 的 CORS（放行 Vercel 前端域名）


### Schema - Jason
概念	说明
Schema	模板、规则、契约（元数据，描述数据）
Model 模型	在 Pydantic 语境，Model 就是 Schema 的代码载体；日常两者混用
Instance 实例	真正的 JSON 数据，按照 Schema 生成出来
JSON‑Schema	通用行业标准格式，用来描述 JSON 数据的 Schema
